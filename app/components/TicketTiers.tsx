...
export default function TicketTiers() {
  return (
    <section id="tickets" className="bg-[var(--cream)] text-black px-6 md:px-24 py-24">
      <p className="font-pixel text-[var(--brand-red)] uppercase tracking-[0.3em] mb-4 text-center">Join the room</p>
          ...
          >
            <div className="flex items-start justify-between mb-6">
              <h3 className="text-2xl font-bold">
                {tier.name.split(' ')[0]}
                {tier.name.includes(' ') && (
                  <span className={tier.featured ? 'text-[var(--brand-red)]' : ''}> {tier.name.split(' ').slice(1).join(' ')}</span>
                )}
              </h3>
              {tier.featured && (
                <span className="bg-[var(--brand-red)] text-white text-[10px] font-bold tracking-widest uppercase px-2 py-1">
                  Popular
                </span>
              )}
            </div>
            ...
            <ul className="space-y-3 mb-10 flex-1">
              {tier.perks.map((perk) => (
                <li key={perk} className="flex gap-2 text-sm">
                  <span className={tier.featured ? 'text-[var(--brand-red)]' : ''}>✓</span>
                  {perk}
                </li>
              ))}
            </ul>
            <Link
              href="/sessions"
              className={`block text-center py-3 text-sm font-bold uppercase tracking-widest ${
                tier.featured
